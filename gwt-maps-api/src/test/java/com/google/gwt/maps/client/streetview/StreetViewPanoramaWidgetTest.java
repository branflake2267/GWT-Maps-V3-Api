...
public class StreetViewPanoramaWidgetTest extends AbstractMapsGWTTest {
	...
	public void testUse() {
		asyncLibTest(new Runnable() {
			...
			public void run() {
				...
				StreetViewPanoramaWidget wStreet = new StreetViewPanoramaWidget(
						options);
	 ...
	 * {
	 * 
	 * final LatLng position = LatLng.newInstance(21.259758694819777,
	 * -157.811758518219);
	 * 
	 * StreetViewPov pov = StreetViewPov.newInstance(); pov.setHeading(0);
	 * pov.setZoom(0); pov.setPitch(0);
	 * 
	 * StreetViewPanoramaOptions options =
	 * StreetViewPanoramaOptions.newInstance(); options.setPosition(position);
	 * options.setStreeViewPov(pov); options.setVisibile(true);
	 * 
	 * options.setPanoProvider(new StreetViewPanoramaProvider() { public
	 * StreetViewPanoramaData getPanoData(String pano, int zoom, int tileX, int
	 * tileY) {
	 * 
	 ...
	 * 
	 * Size tileSize = Size.newInstance(300, 300); Size worldSize =
	 * Size.newInstance(1708, 400);
	 * 
	 * StreetViewTileData tiles = StreetViewTileData.newInstance();
	 * tiles.setCenterHeading(0); tiles.setTileSize(tileSize);
	 * tiles.setWorldSize(worldSize); tiles.getTileUrl(pano, zoom, tileX, tileY,
	 * new TileUrlHandler() { public String getTileUrl(String pano, int zoom,
	 * int tileX, int tileY) { zoom = 0; // TODO make a better tiled pano for
	 * testing String url =
	 * "http://gonevertical-hr.appspot.com/serve?pano=99330&z=" + zoom + "&y=" +
	 * tileY + "&x=" + tileX; System.out.println(url); return url; } });
	 * 
	 * StreetViewPanoramaData data = StreetViewPanoramaData.newInstance();
	 * data.setCopyright("Brandon Donnelson"); data.setLocation(location);
	 * data.setTileData(tiles);
	 * 
	 * return data; } });
	 * 
	 * StreetViewPanoramaWidget wStreet = new StreetViewPanoramaWidget(options);
	 * RootPanel.get().add(wStreet); wStreet.setSize("750px", "500px");
	 * 
			...
			public void run() {
				...
				wStreet.registerPanoProvider(new StreetViewPanoramaProvider() {
					...
					public StreetViewPanoramaData getPanoData(String pano,
						...
						StreetViewPanoramaData svp = StreetViewPanoramaData
								.newInstance();
						StreetViewLocation location = StreetViewLocation
								.newInstance();
