...
public class StyledMapWidget extends Composite {
	...
	private void draw() {
		...
		pWidget.clear();
		pWidget.add(new HTML("<br><div class='styledExample'>Styled Map Example w/ <span>Blue highways</span>, <span>Black landscape</span>, and <span>Red roads</span></div>"));	
		drawMap();
	}
	...
	private void drawMap() {
		...
		style1.setStylers(new MapTypeStyler[] {
				MapTypeStyler.newHueStyler("#CC3232"),
				MapTypeStyler.newSaturationStyler(100),
				MapTypeStyler.newLightnessStyler(-45)
		});
		...
		style2.setStylers(new MapTypeStyler[] {
				MapTypeStyler.newHueStyler("#000000"),
		...
		style3.setStylers(new MapTypeStyler[] {
				MapTypeStyler.newHueStyler("#0000FF"),
        ...
        JsArray<MapTypeStyle> styles = ArrayHelper.toJsArray(array); // JS needs JSArray, convert
        ...
        StyledMapType customMapType = StyledMapType.newInstance(styles, options); // apply to new styled map
		...
		// NOTE: using the xString methods here on Map options b/c it can handle either MapTypeId|string, but Java cannot.
		// 		 If using custom id's, make sure to use the String methods (i.e. in below block)
