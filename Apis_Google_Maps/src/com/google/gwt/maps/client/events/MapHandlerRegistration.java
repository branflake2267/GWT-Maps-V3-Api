...
public class MapHandlerRegistration {
	...
	public static <E extends MapEvent> HandlerRegistration addHandler(
		...
		final JavaScriptObject listener = addHandlerImpl(jso,
				eventType.value(), handler, formatter);
	...
	public static <E extends MapEvent> HandlerRegistration addHandlerMvc(
			JavaScriptObject jso, MapEventType eventType,
			MapHandler<E> handler, MapEventFormatter<E> formatter) {
		final JavaScriptObject listener = addHandlerImplMvc(jso,
				eventType.value(), handler, formatter);
		HandlerRegistration registration = new HandlerRegistration() {
			@Override
			public void removeHandler() {
	...
	public static <E extends MapEvent> HandlerRegistration addHandlerDrawing(
		...
		final JavaScriptObject listener = addHandlerImplDrawing(jso,
				eventType.value(), handler, formatter);
		HandlerRegistration registration = new HandlerRegistration() {
			@Override
			public void removeHandler() {
	...
	public static <E> void addDomListener(JavaScriptObject jso,
			...
			MapHandler<E> handler, boolean capture) {
		addDomListener(jso, type.getName(), handler, capture);
	}

	private static native <E> void addDomListener(JavaScriptObject object,
			String eventName, MapHandler<?> handler, boolean capture) /*-{
		var callback = function(event) {
