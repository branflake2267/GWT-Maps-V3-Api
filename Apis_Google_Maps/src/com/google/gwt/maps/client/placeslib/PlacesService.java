...
public class PlacesService extends JavaScriptObject {
  ...
  /**
   * Retrieves a list of Places in a given area. The PlaceResultss passed to the
   * callback are stripped-down versions of a full PlaceResult. A more detailed
   * PlaceResult for each Place can be obtained by sending a Place Details
   * request with the desired Place's reference value.
   * 
  ...
  /**
   * Similar to the nearbySearch function, with the following differences: it
   * retrieves a list of Places based on the query attribute in the given
   * request object; bounds or location + radius parameters are optional; and
   * the region, when provided, will not restrict the results to places inside
   * the area, only bias the response towards results near it.
   */
  ...
  private static final void processTextSearch(JsArray<PlaceResult> results, String status, PlaceTextSearchHandler handler) {
