...
public class GroundOverlay extends MVCObject<GroundOverlay> {
	 ...
	 * @param options
	 */
	public final static GroundOverlay newInstance(String url, LatLngBounds bounds,
			GroundOverlayOptions options) {
		return createJso(url, bounds, options).cast();
	}

	private final static native JavaScriptObject createJso(String url, LatLngBounds bounds,
			GroundOverlayOptions options) /*-{
	...
	/**
	 * Renders the groundoverlay on the specified map. If map is set to null,
	 * the groundoverlay will be removed.
	 * 
	 * @param mapWidget
	 */
